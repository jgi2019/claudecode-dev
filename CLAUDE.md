...
## PJ切替プロトコル（2026-07-16 確定）
### 入場手続き（PJ初回のみ）
1. TAROが指示文を作成（作成前に該当領域の設計正本＝Notion/jgi-brainを検索・確認する）
2. PJレジストリDBに登録（正位置は `~/Desktop/<PJ名>/` 推奨。調査・戦略系はローカルパスなし可）
3. ラベルは成果物別命名で `[PJ:xxx]`（Slack接頭辞と完全一致させる）

### 起動の型（毎回4手）
1. 既存タブを閉じる
2. `cd <正位置> && claude`
3. `/pj`（引数付き `/pj <PJラベル>` でPJを特定する）
4. 「#taro-jiroのTARO指示【PJ:xxx】を読んで着手」

### 終了の型（毎回2手）
1. 完了報告を #taro-jiro へ（`[PJ:xxx]` 接頭辞必須）
2. タブを閉じる（終了の儀式は存在しない）

### Slack読み分けルール（2026-07-16 オムロン用セッションが他PJ指示を誤読した実害の再発防止）
- 全Slack投稿（TARO⇄JIRO双方向）に `[PJ:xxx]` 接頭辞を必ず付ける
- `/pj <PJラベル>` で起動したセッションは「このセッションは[PJ:xxx]。#taro-jiroでは自PJ宛＋[PJ:ソロプレナーOS]（共通基盤）宛のみ読み、他PJ宛の指示は読み飛ばす」を自己宣言する
- 引数なし `/pj` は現行動作（起動検証のみ）を維持
